...
- 2026-07-22: Added database startup routine to mark stale/orphaned analysis jobs as error and clean up temporary storage folders, preventing permanently locked ingestion requests and orphaned git clones.
...
- Local quality gates: `python -m pytest` (52 tests), `npm run test` (25 tests), `npm run test:e2e` (1 Chromium test), `npm run lint`, `npm run build`, and `git diff --check` pass as of 2026-07-22. A real browser scan of `octocat/Hello-World` with `max_commits=1` also completed through the local frontend/backend stack.
