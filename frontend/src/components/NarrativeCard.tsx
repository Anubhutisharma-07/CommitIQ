    ...
    <div className="glass-panel rounded-[28px] overflow-hidden shadow-2xl relative border border-white/10 flex flex-col justify-between">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 relative z-10">
        <div className="flex items-center gap-2">
          ...
          <span className="font-head text-[14px] font-semibold text-white uppercase tracking-wider">AI Narrative Analyst</span>
        </div>
      ...
      <div className="px-6 py-6 min-h-[140px] relative z-10">
        {state === 'idle' && (
          <div className="flex flex-col gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="text-white text-sm font-semibold mb-1">Generate Intelligence Narrative</h4>
                <p className="text-slate-400 text-xs leading-relaxed max-w-[480px]">
                  Request an AI-generated natural language report explaining the architectural health fluctuations, code coupling anomalies, and refactor opportunities.
                </p>
              </div>
            </div>
        ...
        {state === 'loading' && (
          <div className="flex flex-col items-center justify-center py-6 gap-3 text-slate-400">
            ...
            <p className="text-xs font-medium animate-pulse">Consulting codebase knowledge models...</p>
          </div>
        )}
        ...
        {state === 'error' && (
          <div className="flex items-start gap-3 bg-rose-500/10 border border-rose-500/20 rounded-[20px] p-4 text-xs text-rose-300">
            ...
            <div>
              ...
              <p className="text-slate-400 leading-relaxed mb-2">{error || 'Could not compile explanation.'}</p>
              <button onClick={() => setState('idle')} className="underline text-purple-300 font-semibold hover:text-white transition-colors">
                Try Again
              </button>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between px-6 py-3.5 border-t border-white/5 bg-white/[0.01] relative z-10 text-[10px] text-slate-500 font-mono">
          <div className="flex items-center gap-3">
            {meta.provider && PROVIDER_BADGE[meta.provider] ? (
              <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-md border font-mono font-bold text-[9px] ${PROVIDER_BADGE[meta.provider].color}`}>
                <span className={`w-1.5 h-1.5 rounded-full ${PROVIDER_BADGE[meta.provider].dot}`} />
                {PROVIDER_BADGE[meta.provider].label}
              </span>
            ) : (
              <span className="bg-white/5 px-2 py-1 rounded-md border border-white/10 font-mono font-bold text-[9px]">{meta.model || 'model unavailable'}</span>
            )}
          ...
          <div className="flex items-center gap-3">
            ...
            {meta.cached && <span className="text-emerald-400 bg-emerald-500/10 px-1.5 py-0.2 rounded-md font-bold text-[9px] border border-emerald-500/10">CACHED</span>}
            {meta.demo_mode && <span className="text-blue-400 bg-blue-500/10 px-1.5 py-0.2 rounded-md font-bold text-[9px] border border-blue-500/10">DEMO MODE</span>}
          </div>
        </div>
      )}
    </div>
