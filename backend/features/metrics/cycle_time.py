...
async def compute_cycle_time_metrics(db: AsyncSession, repo_id: int) -> dict:
    stmt = select(PullRequest).where(PullRequest.repo_id == repo_id, PullRequest.state.in_(["closed", "merged", "all"]))
    ...
    avg_cycle = (total_cycle_time.total_seconds() / valid_cycle_prs / 3600) if valid_cycle_prs > 0 else 0.0
